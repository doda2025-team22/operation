...
| Sufficient | All aspects are installed through the central Helm chart (see A3). | [ ]        |  |                              |
| Good | One of the described use cases has been realized. It generally works, but falls short in some aspects. For example, instead of an individual, user-based rate-limit, only a global rate-limit exists that affects everyone. | [X]        |  | A global rate-limit is used. |
