...
|-------------------|-------------|-------|------|-------------|
| **Kubernetes Usage** |  |  |  |  |
| Insufficient | The deployment does not contain a Deployment or a Service or deploying to Kubernetes fails. | [ ] |  |  |
| Poor | The deployment is successful, but can only be accessed through a node port or a service tunnel. | [ ] |  |  |
| Sufficient | The application can be deployed to a Kubernetes cluster. | [ ] |  |  |
...
| Good | The deployed application defines the location of the model service through an environment variable. | [ ] |  |  |
...
| Excellent | All VMs mount the same shared VirtualBox folder as /mnt/shared into the VM. | [ ] |  |  |
